...
#define GET_MAX_ELEMENT_ARRAY_INDEX(type) do {             \
    type *data = (type *)indices_buffer_priv->data;        \
    for (int i = 0; i < indices_buffer_priv->count; i++) { \
        if (data[i] > s->max_element_array_index)          \
            s->max_element_array_index = data[i];          \
    }                                                      \
} while (0)                                                \
...
static int geometry_init(struct ngl_node *node)
{
    ...
    if (s->indices_buffer) {
        ...
        case NGLI_FORMAT_R16_UNORM: GET_MAX_ELEMENT_ARRAY_INDEX(uint16_t); break;
        case NGLI_FORMAT_R32_UINT:  GET_MAX_ELEMENT_ARRAY_INDEX(uint32_t); break;
        default:
            ngli_assert(0);
        }
    }
